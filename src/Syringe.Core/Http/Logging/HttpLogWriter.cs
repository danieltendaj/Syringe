...
namespace Syringe.Core.Http.Logging
{
	public class HttpLogWriter
	{
		...
		public virtual void AppendRequest(Uri uri, HttpRequestMessage request)
		{
			...
			_writer.WriteLine(request.Content.ReadAsStringAsync());
			_writer.WriteLine();
		}

		public virtual void AppendResponse(HttpResponse response)
		{
			...
			{
				_writer.WriteLine(HEADER_FORMAT, header.Key, header.Value);
			}
